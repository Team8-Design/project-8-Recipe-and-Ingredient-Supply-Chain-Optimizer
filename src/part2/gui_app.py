...
def run_gui(bst: RecipeBST, supply_graph: SupplyGraph, heap: UrgencyMaxHeap, sub_graph: SubstitutionGraph, supplier_map: dict = None):
    root = tk.Tk()
    ...
    tab1 = ttk.Frame(content_frame)
    tab2 = ttk.Frame(content_frame)
    tab3 = ttk.Frame(content_frame)
    tab4 = ttk.Frame(content_frame)
    tab5 = ttk.Frame(content_frame)
    tab6 = ttk.Frame(content_frame)
    tab7 = ttk.Frame(content_frame)
    tab8 = ttk.Frame(content_frame)
    tab9 = ttk.Frame(content_frame)
    tab10 = ttk.Frame(content_frame)
    tab11 = ttk.Frame(content_frame)
    
    tabs = [tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10, tab11]
    tab_names = [
        "Recipe Search (食谱搜索)",
        "Cuisine Filter (菜系筛选)",
        "Alphabet Range (字母范围搜索)",
        "Ingredient Substitution (食材替换)",
        "Supply Network (供应网络)",
    ...
    current_tab = tk.IntVar(value=0)
    
    def switch_tab(tab_index):
        current_tab.set(tab_index)
    ...
    for i, name in enumerate(tab_names):
        row = i // 6
        col = i % 6
        ttk.Button(tab_bar, text=name, 
                   command=lambda idx=i: switch_tab(idx)).grid(row=row, column=col, sticky="ew")
    ...
    def create_visualization_container(parent, draw_func, default_size=(10, 6)):
        ...
        info_label = ttk.Label(control_frame, text="Select node from dropdown below", foreground='#636E72')
        ...
        ttk.Label(bottom_panel, text="Select Node:").pack(side=tk.LEFT, padx=5)
        
        node_var = tk.StringVar()
        node_combobox = ttk.Combobox(bottom_panel, textvariable=node_var, state='readonly', width=40)
        node_combobox.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(bottom_panel, text="View Details", command=lambda: on_view_details()).pack(side=tk.LEFT, padx=5)
        ...
        def render_plot(highlight_item=None):
            nonlocal current_img, last_fig, click_data, img_w, img_h
            ...
            if highlight_item:
                for item in click_data:
                    if item['type'] == highlight_item['type'] and item['id'] == highlight_item['id']:
                        ax.scatter(item['x'], item['y'], s=item['radius'] * 1000 * 4, 
                                   c='none', edgecolors='#FF0000', linewidth=3, zorder=20)
                        ax.scatter(item['x'], item['y'], s=item['radius'] * 1000 * 2, 
                                   c='none', edgecolors='#FF0000', linewidth=2, zorder=20)
                        break
        ...
        def update_combobox():
            if not click_data:
                node_combobox['values'] = []
                return
            
            items = []
            for item in click_data:
                if item['type'] == 'supplier':
                    items.append(f"S{item['id']} - {item['name']}")
                elif item['type'] == 'ingredient':
                    items.append(f"I{item['id']} - {item['name']}")
                elif item['type'] == 'heap_node':
                    items.append(f"Heap[{item['id']}] - {item['name']}")
                elif item['type'] == 'bst_node':
                    items.append(f"BST #{item['id']} - {item['name']}")
                elif item['type'] == 'll_node':
                    items.append(f"LL #{item['id']} - {item['name']}")
                else:
                    items.append(f"{item['id']} - {item['name']}")
            node_combobox['values'] = items
        
        def get_selected_item():
            selected_text = node_var.get()
            if not selected_text or not click_data:
                return None
            
            for item in click_data:
                if item['type'] == 'supplier':
                    expected = f"S{item['id']} - {item['name']}"
                elif item['type'] == 'ingredient':
                    expected = f"I{item['id']} - {item['name']}"
                elif item['type'] == 'heap_node':
                    expected = f"Heap[{item['id']}] - {item['name']}"
                elif item['type'] == 'bst_node':
                    expected = f"BST #{item['id']} - {item['name']}"
                elif item['type'] == 'll_node':
                    expected = f"LL #{item['id']} - {item['name']}"
                else:
                    expected = f"{item['id']} - {item['name']}"
        ...
        def on_view_details():
            ...
            if not selected_item:
                selected_item = get_selected_item()
            if selected_item:
                show_detail_popup(selected_item)
        
        def show_detail_popup(item):
            popup = tk.Toplevel(parent)
            popup.title(f"Details - {item['type'].replace('_', ' ').title()}")
            popup.geometry("300x350")
            popup.resizable(False, False)
            popup.transient(parent)
            
            main_frame = ttk.LabelFrame(popup, text="Detailed Information", padding=10)
            ...
            if item['type'] == 'supplier':
                fields = [
                    ("ID", f"S{item['id']}"),
                    ("Name", item['name']),
                    ("Reliability", item['reliability'].replace('Reliability: ', '') if item['reliability'] else "N/A"),
                    ("Location", item['location'].replace('Location: ', '') if item['location'] else "N/A")
                ]
            elif item['type'] == 'ingredient':
                fields = [
                    ("ID", f"I{item['id']}"),
                    ("Name", item['name']),
                    ("Category", item['category'].replace('Category: ', '') if item['category'] else "N/A")
                ]
            elif item['type'] == 'heap_node':
                fields = [
                    ("Heap Index", item['id']),
                    ...
                    ("Category", item['category'].replace('Category: ', '') if item['category'] else "N/A"),
                    ("Urgency", item['urgency'].replace('Urgency: ', '') if item['urgency'] else "N/A"),
                    ("Stock", item['stock'].replace('Stock: ', '') if item['stock'] else "N/A")
                ]
            elif item['type'] == 'bst_node':
                fields = [
                    ("Recipe ID", f"#{item['id']}"),
                    ("Recipe Name", item['name']),
                    ("Cuisine", item['category'].replace('Cuisine: ', '') if item['category'] else "N/A"),
                    ("Difficulty", item['difficulty'].replace('Difficulty: ', '') if item['difficulty'] else "N/A"),
                    ("Servings", item['servings'].replace('Servings: ', '') if item['servings'] else "N/A")
                ]
            elif item['type'] == 'll_node':
                fields = [
                    ("Ingredient ID", f"#{item['id']}"),
                    ("Ingredient Name", item['name']),
                    ("Category", item['category'].replace('Category: ', '') if item['category'] else "N/A"),
                    ("Quantity", item['quantity'].replace('Quantity: ', '') if item['quantity'] else "N/A"),
                    ("Stock", item['stock'].replace('Stock: ', '') if item['stock'] else "N/A"),
                    ("Position", item['position'].replace('Position: ', '') if item['position'] else "N/A")
                ]
            else:
                fields = [
                    ("ID", item['id']),
            ...
            for label_text, value in fields:
                ttk.Label(main_frame, text=f"{label_text}:", font=('Arial', 10, 'bold'), foreground='#2D3436').grid(row=row, column=0, sticky=tk.W, pady=3)
                ttk.Label(main_frame, text=str(value), font=('Arial', 10), foreground='#636E72').grid(row=row, column=1, sticky=tk.W, pady=3, padx=10)
                row += 1
            
            ttk.Button(main_frame, text="Close", command=popup.destroy).grid(row=row, column=0, columnspan=2, pady=10)
        
        node_combobox.bind('<<ComboboxSelected>>', on_node_select)
    ...
    def draw_supply_graph(ax):
        ...
        node_size = max(300, min(600, 2500 / max(n_suppliers, n_ingredients, 1)))
        
        hover_data = []
        
        for sid in suppliers:
            x, y = supplier_pos[sid]
            ax.scatter(x, y, s=node_size, c='#636E72', edgecolors='#2D3436', 
                       zorder=5, marker='s')
            
            sup_name = ""
            sup_reliability = ""
            sup_location = ""
            if supplier_map and sid in supplier_map:
                sup = supplier_map[sid]
                sup_name = sup.name[:8]
                sup_reliability = f"Reliability: {sup.reliability_score:.2f}"
                sup_location = f"Location: {sup.location}"
            ...
            ax.text(x, y, label_text, fontsize=min(8, 200 / max_items), 
                    ha='center', va='center', fontweight='bold', color='white')
            
            hover_data.append({
                'type': 'supplier',
                'id': sid,
                'name': supplier_map[sid].name if (supplier_map and sid in supplier_map) else f'Supplier {sid}',
                'reliability': sup_reliability,
                'location': sup_location,
                'x': x,
                'y': y,
                'radius': node_size ** 0.5 / 50
            })
        
        for iid in ingredients:
            x, y = ingredient_pos[iid]
            ax.scatter(x, y, s=node_size, c='#00B894', edgecolors='#00CEC9', 
                       zorder=5, marker='o')
            
            ing_name = ""
            ing_category = ""
            if sub_graph and iid in sub_graph.ingredient_map:
                ing = sub_graph.ingredient_map[iid]
                ing_name = ing.name[:8]
                ing_category = f"Category: {ing.category.value}"
            ...
            hover_data.append({
                'type': 'ingredient',
                'id': iid,
                'name': sub_graph.ingredient_map[iid].name if (sub_graph and iid in sub_graph.ingredient_map) else f'Ingredient {iid}',
    ...
    def draw_heap(ax):
        ...
        for idx in range(n):
            ...
            ax.text(x, y - 0.02, f'ID:{idx}', fontsize=7, ha='center', va='top', color='#636E72')
            ax.text(x, y, f'{urgency:.0f}', fontsize=10, ha='center', va='center', 
                    fontweight='bold', color='white')
            ax.text(x, y + 0.025, f'{ing.name[:10]}', fontsize=7, ha='center', va='bottom', color='#2D3436')
            ...
            right = 2 * idx + 2
            if left < n:
                ax.plot([x, pos[left][0]], [y, pos[left][1]], '#636E72', linewidth=2)
            if right < n:
                ax.plot([x, pos[right][0]], [y, pos[right][1]], '#636E72', linewidth=2)
            
            hover_data.append({
                'type': 'heap_node',
                'id': idx,
                'name': ing.name,
                'category': f"Category: {ing.category.value}",
                'urgency': f"Urgency: {urgency:.2f}",
                'stock': f"Stock: {ing.current_stock}/{ing.minimum_stock}{ing.unit}",
                'x': x,
                'y': y,
                'radius': node_size ** 0.5 / 50
            })
    ...
    def draw_bst(ax):
        ...
        def draw_tree(node):
            ...
            ax.scatter(x, y, s=500, c=color, edgecolors=edge_color, zorder=5)
            ax.text(x, y - 0.015, f'#{node.data.recipe_id}', fontsize=7, ha='center', va='center', fontweight='bold', color='#2D3436', zorder=10)
            ax.text(x, y + 0.015, f'{node.data.name[:6]}', fontsize=7, ha='center', va='center', fontweight='bold', color='#2D3436', zorder=10)
            
            if node == bst.root:
                ax.scatter(x, y + 0.06, s=100, c='#E17055', marker='*', zorder=10)
                ax.text(x, y + 0.09, 'ROOT', fontsize=7, ha='center', va='bottom', color='#E17055', fontweight='bold', zorder=10)
            
            if node.left:
                lx, ly = node_info[node.left]['pos']
                ax.plot([x, lx], [y, ly], '#636E72', linewidth=2)
                ax.text((x + lx) / 2, (y + ly) / 2 + 0.01, 'L', fontsize=8, ha='center', va='bottom', color='#00B894', fontweight='bold', zorder=10)
                draw_tree(node.left)
            
            if node.right:
                rx, ry = node_info[node.right]['pos']
                ax.plot([x, rx], [y, ry], '#636E72', linewidth=2)
                ax.text((x + rx) / 2, (y + ry) / 2 + 0.01, 'R', fontsize=8, ha='center', va='bottom', color='#E17055', fontweight='bold', zorder=10)
            ...
            hover_data.append({
                ...
                'id': recipe.recipe_id,
                'name': recipe.name,
                'category': f"Cuisine: {recipe.cuisine_type.value}",
                'difficulty': f"Difficulty: {recipe.difficulty.value}",
    ...
    def draw_linked_list(ax):
        ...
        for i, (ing, qty) in enumerate(ingredients):
            ...
            ax.text(x, y_center - 0.04, f'#{ing.ingredient_id}\n{ing.name[:8]}', fontsize=8, ha='center', va='center', fontweight='bold', color='white')
            ...
            hover_data.append({
                ...
                'name': ing.name,
                'category': f"Category: {ing.category.value}",
                'quantity': f"Quantity: {qty}{ing.unit}",
                'stock': f"Stock: {ing.current_stock}/{ing.minimum_stock}{ing.unit}",
                'position': f"Position: {i+1}/{n}",
                'x': x,
                'y': y_center,
                'radius': 0.03
            })
        ...
        ax.legend(handles=legend_elements, loc='upper right', bbox_to_anchor=(0.98, 0.98), 
                  ncol=2, fontsize=9, frameon=True, edgecolor='lightgray')
    ...
    # Tab11: Weekly Menu (每周菜单)
    weekly_menu_notebook = ttk.Notebook(tab11)
    weekly_menu_notebook.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

    menu_tab1 = ttk.Frame(weekly_menu_notebook)
    menu_tab2 = ttk.Frame(weekly_menu_notebook)
    menu_tab3 = ttk.Frame(weekly_menu_notebook)
    menu_tab4 = ttk.Frame(weekly_menu_notebook)

    weekly_menu_notebook.add(menu_tab1, text="Stock Status (库存状态)")
    weekly_menu_notebook.add(menu_tab2, text="Available Recipes (可用食谱)")
    weekly_menu_notebook.add(menu_tab3, text="This Week Menu (本周菜单)")
    weekly_menu_notebook.add(menu_tab4, text="Next Week Menu (下周菜单)")

    from src.part2.menu_planner import WeeklyMenuPlanner
    planner = WeeklyMenuPlanner(bst, heap, supply_graph)

    # Tab11.1: Stock Status (库存状态)
    stock_tree = ttk.Treeview(menu_tab1, columns=('ID', 'Name', 'Category', 'Current', 'Min', 'Unit', 'Status'), show='headings')
    stock_tree.heading('ID', text='ID (食材ID)')
    stock_tree.heading('Name', text='Name (食材名称)')
    stock_tree.heading('Category', text='Category (分类)')
    stock_tree.heading('Current', text='Current (当前库存)')
    stock_tree.heading('Min', text='Min (最低库存)')
    stock_tree.heading('Unit', text='Unit (单位)')
    stock_tree.heading('Status', text='Status (状态)')
    
    stock_tree.column('ID', width=60)
    stock_tree.column('Name', width=120)
    stock_tree.column('Category', width=100)
    stock_tree.column('Current', width=80)
    stock_tree.column('Min', width=80)
    stock_tree.column('Unit', width=60)
    stock_tree.column('Status', width=80)
    
    stock_scroll = ttk.Scrollbar(menu_tab1, orient=tk.VERTICAL, command=stock_tree.yview)
    stock_tree.configure(yscroll=stock_scroll.set)
    ...
    def refresh_stock_status():
        ...
        all_ingredients = []
        for rec in bst.in_order():
            if rec.ingredient_list:
                for ing, _ in rec.ingredient_list.get_all_ingredients():
        ...
        for ing in all_ingredients:
            ...
            stock_tree.insert('', tk.END, values=(
                ...
                ing.name,
                ing.category.value,
                f"{ing.current_stock:.2f}",
                f"{ing.minimum_stock:.2f}",
                ing.unit,
                status
            ))
    ...
    # Tab11.2: Available Recipes (可用食谱)
    recipe_tree = ttk.Treeview(menu_tab2, columns=('ID', 'Name', 'Cuisine', 'Difficulty', 'Servings', 'Feasible'), show='headings')
    recipe_tree.heading('ID', text='ID (食谱ID)')
    recipe_tree.heading('Name', text='Name (食谱名称)')
    recipe_tree.heading('Cuisine', text='Cuisine (菜系)')
    recipe_tree.heading('Difficulty', text='Difficulty (难度)')
    recipe_tree.heading('Servings', text='Servings (份量)')
    recipe_tree.heading('Feasible', text='Feasible (是否可行)')
    
    recipe_tree.column('ID', width=60)
    recipe_tree.column('Name', width=150)
    recipe_tree.column('Cuisine', width=100)
    recipe_tree.column('Difficulty', width=80)
    recipe_tree.column('Servings', width=60)
    recipe_tree.column('Feasible', width=80)
    
    recipe_scroll = ttk.Scrollbar(menu_tab2, orient=tk.VERTICAL, command=recipe_tree.yview)
    recipe_tree.configure(yscroll=recipe_scroll.set)
    recipe_scroll.pack(side=tk.RIGHT, fill=tk.Y)
    recipe_tree.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    ...
    def refresh_recipes():
        ...
        for rec in all_recipes:
            ...
            status = "✓ Feasible (可行)" if feasible else "✗ Missing (缺食材)"
            
            recipe_tree.insert('', tk.END, values=(
                rec.recipe_id,
                rec.name,
                rec.cuisine_type.value,
                rec.difficulty.value,
                rec.servings,
                status
            ))
    ...
    # Tab11.3: This Week Menu (本周菜单)
    this_week_frame = ttk.Frame(menu_tab3)
    this_week_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

    this_week_tree = ttk.Treeview(this_week_frame, columns=('Day', 'DayCN', 'RecipeID', 'RecipeName', 'Cuisine', 'Difficulty', 'Status', 'Missing'), show='headings')
    this_week_tree.heading('Day', text='Day (星期)')
    this_week_tree.heading('DayCN', text='Date (日期)')
    this_week_tree.heading('RecipeID', text='RecipeID (食谱ID)')
    this_week_tree.heading('RecipeName', text='RecipeName (食谱名称)')
    this_week_tree.heading('Cuisine', text='Cuisine (菜系)')
    this_week_tree.heading('Difficulty', text='Difficulty (难度)')
    this_week_tree.heading('Status', text='Status (库存状态)')
    this_week_tree.heading('Missing', text='Missing (缺口食材)')
    
    this_week_tree.column('Day', width=80)
    this_week_tree.column('DayCN', width=60)
    this_week_tree.column('RecipeID', width=60)
    this_week_tree.column('RecipeName', width=150)
    this_week_tree.column('Cuisine', width=100)
    this_week_tree.column('Difficulty', width=80)
    this_week_tree.column('Status', width=80)
    this_week_tree.column('Missing', width=200)
    
    this_week_scroll = ttk.Scrollbar(this_week_frame, orient=tk.VERTICAL, command=this_week_tree.yview)
    this_week_tree.configure(yscroll=this_week_scroll.set)
    this_week_scroll.pack(side=tk.RIGHT, fill=tk.Y)
    this_week_tree.pack(fill=tk.BOTH, expand=True)
    ...
    def update_menu_tree(tree, menu_data):
        tree.delete(*tree.get_children())
        
        for day_plan in menu_data:
            day = day_plan['day']
            day_cn = day_cn_map.get(day, day)
            recipe = day_plan['recipe']
            feasible = day_plan['feasible']
            missing = day_plan['missing']
            ...
            if recipe:
                ...
                if feasible:
                    ...
                    status = "✗ Missing (缺食材)"
                    missing_text = ", ".join([f"{m.name}" for m in missing]) if missing else ""
            else:
                recipe_id = "-"
                recipe_name = "No Recipe (无可用食谱)"
                cuisine = "-"
                difficulty = "-"
                status = "-"
                missing_text = ""
    ...
    def generate_this_week_menu():
        global current_menu
        try:
            current_menu = planner.generate_weekly_menu(week_offset=0)
            update_menu_tree(this_week_tree, current_menu)
        except Exception as e:
            messagebox.showerror("Error (错误)", f"Failed to generate menu: {str(e)}")

    def regenerate_this_week_menu():
        generate_this_week_menu()

    this_week_button_frame = ttk.Frame(menu_tab3)
    this_week_button_frame.pack(fill=tk.X, padx=10, pady=5)
    ttk.Button(this_week_button_frame, text="Generate This Week Menu (生成本周菜单)", command=generate_this_week_menu).pack(side=tk.LEFT, padx=5)
    ttk.Button(this_week_button_frame, text="Regenerate (重新生成)", command=regenerate_this_week_menu).pack(side=tk.LEFT, padx=5)

    generate_this_week_menu()

    # Tab11.4: Next Week Menu (下周菜单)
    next_week_frame = ttk.Frame(menu_tab4)
    next_week_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

    next_week_tree = ttk.Treeview(next_week_frame, columns=('Day', 'DayCN', 'RecipeID', 'RecipeName', 'Cuisine', 'Difficulty', 'Status', 'Missing'), show='headings')
    next_week_tree.heading('Day', text='Day (星期)')
    next_week_tree.heading('DayCN', text='Date (日期)')
    next_week_tree.heading('RecipeID', text='RecipeID (食谱ID)')
    next_week_tree.heading('RecipeName', text='RecipeName (食谱名称)')
    next_week_tree.heading('Cuisine', text='Cuisine (菜系)')
    next_week_tree.heading('Difficulty', text='Difficulty (难度)')
    next_week_tree.heading('Status', text='Status (库存状态)')
    next_week_tree.heading('Missing', text='Missing (缺口食材)')
    
    next_week_tree.column('Day', width=80)
    next_week_tree.column('DayCN', width=60)
    next_week_tree.column('RecipeID', width=60)
    next_week_tree.column('RecipeName', width=150)
    next_week_tree.column('Cuisine', width=100)
    next_week_tree.column('Difficulty', width=80)
    next_week_tree.column('Status', width=80)
    next_week_tree.column('Missing', width=200)
    
    next_week_scroll = ttk.Scrollbar(next_week_frame, orient=tk.VERTICAL, command=next_week_tree.yview)
    next_week_tree.configure(yscroll=next_week_scroll.set)
    next_week_scroll.pack(side=tk.RIGHT, fill=tk.Y)
    next_week_tree.pack(fill=tk.BOTH, expand=True)

    next_week_menu = []

    def generate_next_week_menu():
        global next_week_menu
        try:
            next_week_menu = planner.generate_next_week_menu()
            update_menu_tree(next_week_tree, next_week_menu)
        except Exception as e:
            messagebox.showerror("Error (错误)", f"Failed to generate next week menu: {str(e)}")

    def regenerate_next_week_menu():
        generate_next_week_menu()

    next_week_button_frame = ttk.Frame(menu_tab4)
    next_week_button_frame.pack(fill=tk.X, padx=10, pady=5)
    ttk.Button(next_week_button_frame, text="Generate Next Week Menu (推算下周菜单)", command=generate_next_week_menu).pack(side=tk.LEFT, padx=5)
    ttk.Button(next_week_button_frame, text="Regenerate (重新生成)", command=regenerate_next_week_menu).pack(side=tk.LEFT, padx=5)
