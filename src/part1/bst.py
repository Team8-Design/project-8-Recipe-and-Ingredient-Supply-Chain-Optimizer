...
class RecipeBST:
    ...
    def delete(self, recipe_name: str, case_sensitive: bool = False) -> bool:
        ...
        # 递归删除核心逻辑
        def _delete(node: BSTNode, target: str) -> BSTNode | None:
            if not node:
                return None
            ...
            if target_lower < node_name_lower:
                ...
                # 删除右子树最小节点
                node.right = _delete(node.right, min_right.data.name)
            return node

        original_root = self.root
        self.root = _delete(self.root, recipe_name)
        # 若根节点变化，说明删除成功；否则对比是否有节点被删
