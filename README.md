...
- **Visualization**: Interactive GUI with 11 tabs (two rows), data structure visualization, performance charts
- **Testing**: 122+ comprehensive unit tests covering edge cases
...
The system includes a comprehensive Tkinter GUI with **11 tabs arranged in two rows**:

**First Row (上排)**:
| Tab | Name | Description |
|-----|------|-------------|
| Tab1 | Recipe Search (食谱搜索) | Search recipe by name, view detailed information |
| Tab2 | Cuisine Filter (菜系筛选) | Filter recipes by cuisine type (Chinese/Western/Japanese/Thai) |
| Tab3 | Alphabet Range (字母范围搜索) | Search recipes by alphabet letter range (A-F, G-M, etc.) |
| Tab4 | Ingredient Substitution (食材替换) | Query substitute ingredients for a given ingredient ID |
| Tab5 | Supply Network (供应网络) | Query suppliers for a given ingredient ID |
| Tab6 | Performance Chart (性能图表) | Display benchmark results with theoretical complexity curves |

**Second Row (下排)**:
| Tab | Name | Description |
|-----|------|-------------|
| Tab7 | Test Results (测试结果) | Run all tests with detailed report and progress bar |
| Tab8 | Data Management (数据管理) | Add/delete recipes, ingredients, suppliers, and substitutions |
| Tab9 | Structure Visualization (结构可视化) | Visualize data structures with zoom functionality |
| Tab10 | Crisis Recovery (危机恢复) | Simulate supplier shutdown, assess risk, generate recovery plans |
| Tab11 | Weekly Menu (每周菜单) | Stock status, available recipes, weekly menu planning |
...
|-----------|------------|------------|
| Top-Left | BST Search (O(logN)), Graph Search (O(1)), Linked List Iter (O(K)) | Nanoseconds (ns) |
| Top-Right | BST Insert (O(logN)), Supplier Optimization (O(M*S)) | Microseconds (μs) |
| Bottom-Left | Heap Top-10 (O(logN)), BST In-order (O(N)) | Milliseconds (ms) |
| Bottom-Right | All Recipe Feasibility Check (O(N*K)) | Milliseconds (ms) |

**Features**:
- Logarithmic Y-axis to display multiple orders of magnitude
- Data point annotations with scientific notation
- **Theoretical complexity curves** (dashed lines) for comparison
- Complexity labels in legend (O(logN), O(N), O(M*S))
...
Contains 4 sub-tabs for weekly menu planning based on current stock:

| Sub-tab | Name | Description |
|---------|------|-------------|
| Tab11.1 | Stock Status (库存状态) | Display all ingredients with current stock, minimum stock, and status (Adequate/Shortage/Out of Stock) |
| Tab11.2 | Available Recipes (可用食谱) | List all recipes with feasibility status (✓ Feasible / ✗ Missing) |
| Tab11.3 | This Week Menu (本周菜单) | Generate weekly menu (Mon-Sun) based on current stock availability |
| Tab11.4 | Next Week Menu (下周菜单) | Generate forecasted menu for the following week |

**Features**:
- **Generate Weekly Menu**: Automatically assign recipes to each day, prioritizing feasible ones
- **Regenerate**: Randomly reassign recipes for variety
- **Next Week Forecast**: Predict menu for the following week
- **Feasibility Status**: Color-coded indicators for stock availability
- **Missing Ingredients**: Clear display of ingredients needed for infeasible recipes
...
- **Weekly Menu Planner**: Analyze menu, generate procurement plans, generate weekly menu, next week forecast
